...
class Ssh(object):
    def __init__(self, ip, username, password, timeout=30):
        ...
        :param ip: 连接ssh的ip地址，可以是hostname
        ...
        # 连接服务器 self.ssh.connect(hostname='192.168.1.103', port=22, username='root', password='teamwork@123')
        self.ssh.connect(hostname=self.ip, port=22, username=self.username, password=self.password)

    def send_origin(self, cmd):
        '''
        发送要执行的命令
    ...
    def send(self, cmd, notice=False):
        '''
        发送要执行的命令
